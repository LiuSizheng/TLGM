...
clear;
net=load('yeast1458.txt'); %读取网络文件test0425
...
average_i=sir_main(adjMat,1000,0.14) ;%传播概率 每个节点的平均感染数0.047

save('yeast1458.mat', 'd', 'h','p','MDD_core','kcore','neighbor_Core','GAC','KSGC','average_i');

average_iall={};
w = waitbar(0, 'Please wait...'); % 初始化进度条

for i = 1:30
    waitbar(i/30,w , sprintf('Progress: %d %%', floor(i/30*100))); % 更新进度条
    disp(i);
...
save('yeast1458_sir1_30_Kendall.mat', 'a00_degree', 'a00_h_index','a00_pagerank','a00_MDD_core','a00_GAC','a00_kcore','a00_neighbor_core','a00_KSGC');
