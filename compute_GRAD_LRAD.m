...
%%读取网络文件，找到最大连通分量
clear;
net=load('LFR2000-k15.txt'); %读取网络文件
...
BC = compute_betweenness(adjMat)'*2/((N-1)*(N-2));%介数中心性
CC=closeness(adjMat);%接近中心性
%ISBC = compute_ISBC(adjMat);
%ISC=compute_ISC(adjMat);
GRAD = compute_GRAD(adjMat);
%GRACC = compute_GRACC(adjMat);
diameter = calculate_diameter(adjMat);
L = ceil(diameter / 2); % 向上取整，以确保包含足够的邻居

LRAD=compute_LRAD(adjMat, L);
%LRACC=compute_LRACC(adjMat, L);

save('LFR2000-k15_GRAD_LRAD.mat', 'GRAD','LRAD');
% for i=1:N
...
% 
% 
% % average_i=sir_main(adjMat,1000,0.125) ;%传播概率 每个节点的平均感染数0.047
% % 
% % save('netscience_GAC_GRA.mat', 'GAC','GRAD','GRACC');
% 
% average_iall={};
% w = waitbar(0, 'Please wait...'); % 初始化进度条
% 
% for i = 1:26
%     waitbar(i/26,w , sprintf('Progress: %d %%', floor(i/26*100))); % 更新进度条
%     disp(i);
...
%     a00_GRAD(i)=corr(average_iall{i}',GRAD','type','Kendall');
%     a00_GRACC(i)=corr(average_iall{i}',GRACC','type','Kendall');
%     a00_LRAD(i)=corr(average_iall{i}',LRAD','type','Kendall');
%     a00_LRACC(i)=corr(average_iall{i}',LRACC','type','Kendall');
% end
% 
% save('netscience_GAC_GRA_sir1_26_Kendall.mat', 'average_iall','a00_degree', 'a00_h_index','a00_pagerank','a00_MDD_core','a00_GAC','a00_kcore','a00_neighbor_core','a00_KSGC','a00_GRAD','a00_GRACC','a00_LRAD','a00_LRACC');
% 
