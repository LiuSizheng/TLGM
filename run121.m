...
% bata=0.05;recover=1;run_number=100;pp=1
% for i=1:50
%     disp(i);
%     sa=state(1,1:pp);
%     
%     
%     for m=1:run_number
%         [r_result,i_result]=sir_mutiple_initial_set(N,BV,sa,bata,recover);
%         r1(m)=r_result;
%     end
...
%     
%    for m=1:run_number
%         [r_result,i_result]=sir_mutiple_initial_set(N,BV,sr,bata,recover);
%         r2(m)=r_result;
%     end
...
%    
%     for m=1:run_number
%         [r_result,i_result]=sir_mutiple_initial_set(N,BV,sd,bata,recover);
%         r3(m)=r_result;
%     end
...
%     
%     for m=1:run_number
%         [r_result,i_result]=sir_mutiple_initial_set(N,BV,sk,bata,recover);
%         r4(m)=r_result;
%     end
...
%%%%%%%%%%%%%%%%%%%%LIR
% function Result_LIR=LIR(mixedsig,N,DegreeAll)
for i=1:N
    neighbor_of_i= find(mixedsig(i,:));
    Result_LIR(i)=length(find(DegreeAll(neighbor_of_i)>degree(i,mixedsig)));
end
hehe=1:1:N;
hebing_LIR_degree=[Result_LIR;DegreeAll;hehe]';
LIR=sortrows(hebing_LIR_degree,1)';
% end
...
% %
 k=50;
bata=0.01;recover=1;run_number=100;
for i=11:15
    disp(i);
    sa=state(1,1:k);
    for m=1:run_number
   ...
   for m=1:run_number
    ...
    for m=1:run_number
        [r_result,i_result]=sir_mutiple_initial_set(N,BV,sl,bata,recover);
        r5(m)=r_result;
    end
    result_LIR(i)=sum(r5)/run_number;

    bata=bata+0.01;
    
end
result_all=[result_k_shell;result_degree;result_VoteRank;result_run;result_LIR];
bata_all=0.01:0.01:0.15;
figure;
axes('linewidth',1,'box','on','FontSize',13);
xlabel('propagation probablity','FontSize',13);
ylabel('influence spread','FontSize',13);
% grid on;
hold on;
plot(bata_all,result_k_shell,'bo-',bata_all,result_degree,'rs-',bata_all,result_VoteRank,'kp-',bata_all,result_run,'mv-',bata_all,result_LIR,'gd-');
legend('core','degree','VoteRank','run','LIR','Location','southeast');
