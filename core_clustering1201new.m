...
bata=1/k1;recover=1;run_number=1000;
for i=1:k
    disp(i);
    sa=state(1,1:i);
    stack_voterank=zeros(1,n);
    ...
    stack_k_shell(sk)=1;
    for m=1:run_number
        r4(m)=sir_more(A,stack_k_shell,bata,recover);
    end
...
xlabel('初始激活节点集合个数','FontSize',13);
ylabel('感染比例（%）','FontSize',13);
% grid on;
hold on;
