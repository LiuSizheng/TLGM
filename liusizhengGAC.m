...
mix=mixedsig;
M_core=MDD(mixedsig);%混合度分解法（MDD）
...
average_i=sir_main(mixedsig,1000,0.063) ;%传播概率 每个节点的平均感染数
%%%%%%%%%%%%%%%%%%%%%
average_iall={};
for i=1:26
    disp(i);
...
for i=1:15
    ...
    a00_h(i)=corr(average_iall{i}',h,'type','Kendall');%H指数
    a00_pagerank(i)=corr(average_iall{i}',c,'type','Kendall');%PageRank中心性
    a00_M_core(i)=corr(average_iall{i}',M_core','type','Kendall');%混合度分解法MDD
    ...
    a0_core(i)=corr(average_iall{i}',core','type','Kendall');%k壳分解法
