...
%%读取网络文件，找到最大连通分量
clear;
database={'facebook324.txt','netscience.txt','infectious.txt','yeast1458.txt','protain.txt','CA-GrQc.txt'};
name={'facebook','netscience','infectious','yeast','protain','CA-GrQc'};
w = waitbar(0, 'Please wait...'); % 初始化进度条
for o =1:length(database)
    
    net=load(database{o}); %读取网络文件test0425
    ...
    GAC_iall{o}=GAC_lamda; 
    waitbar(o/length(database),w , sprintf('Progress: %d %%', floor(o/length(database)*100))); % 更新进度条
end
close(w); % 运行结束后关闭进度条
...
for pp=1:length(sir_database)
    load(sir_database{pp});

    %average_iall{i}
    for q=1:20
        sum=0;
        for m=1:length(average_iall)
            %corrr{m}=corr(average_iall{m}',GAC_iall{pp}{q}','type','Kendall');
            sum=sum+corr(average_iall{m}',GAC_iall{pp}{q}','type','Kendall');
        end
        average_kendall=sum/m;
        average_kendall_i{q}=average_kendall;
    end
    average_kendall_all{pp}=average_kendall_i;
    figure; 
    x=0.05:0.05:1;
    plot(x, cell2mat(average_kendall_all{pp}), '^-', 'Color', [0, 0, 1], 'MarkerFaceColor', [0, 0, 1], 'DisplayName', 'PRC', 'MarkerSize', 6); % 黑色
    % 添加标题和轴标签
    title(name{pp});
...
save('lambda_test.mat', 'average_kendall_all','GAC_iall');
