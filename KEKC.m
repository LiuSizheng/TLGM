...
for i = 1:N
    first_neighbors = find(mixedsig(i, :));
    EKC_values_1st = EKC_values(first_neighbors);
    EKC_sum=sum(EKC_values_1st);
    KEKC0(i)=core(i)*EKC_sum;
end
...
for i=1:r
    ...
    neighbor_KEKC(i)=0;
    for j=1:len1
        neighbor_KEKC(i)=KEKC0(B(j))+neighbor_KEKC(i);
    end
end
[r,l]=size(mixedsig);
...
% a00_PC=corr(average_i',KEKC0,'type','Kendall');
% a00_neighbor_pc=corr(average_i',neighbor_KEKC','type','Kendall');
% a00_extend_neighbor_pc=corr(average_i',extend_neighbor_KEKC','type','Kendall');
...
for i=7:21
    a00_PC(i)=corr(average_iall{i}',KEKC0,'type','Kendall');
